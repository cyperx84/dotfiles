...
update_claude_config() {
    ...
    if [[ -f "$claude_config" ]]; then
        if command -v jq &> /dev/null; then
            local servers_content=$(cat "$MCP_CONFIG")
            local temp_file=$(mktemp)
            jq --argjson servers "$servers_content" '.mcpServers = $servers' "$claude_config" > "$temp_file"
            mv "$temp_file" "$claude_config"
            echo -e "${GREEN}✅ Claude Code: Embedded server definitions in .claude.json${NC}"
        else
            echo -e "${YELLOW}⚠️  Claude Code: jq not found, manually update mcpServers field in ~/.claude.json${NC}"
            echo -e "${YELLOW}    Copy content from $MCP_CONFIG into the mcpServers field${NC}"
        fi
    else
